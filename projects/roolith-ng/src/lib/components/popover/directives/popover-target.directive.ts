import { Directive, ElementRef, inject, input, OnInit } from '@angular/core';

@Directive({
  selector: '[rngPopoverTarget]',
})
export class PopoverTargetDirective implements OnInit {
  public rngPopoverTarget = input<string>('');

  private _element = inject(ElementRef);

  ngOnInit(): void {
    this._setAttribute();
  }

  /**
   * Get the first child element of the host element, which is expected to be the dropdown trigger element.
   *
   * @returns HTMLElement | null
   */
  private _getFirstClickableElement(): HTMLElement | null {
    const element = this._element.nativeElement as HTMLElement;

    if (element.tagName === 'BUTTON' || element.tagName === 'A') {
      return element;
    }

    if (element.firstElementChild?.tagName !== 'BUTTON') {
      return element.firstElementChild?.querySelector('button') as HTMLElement | null;
    }

    return element.firstElementChild as HTMLElement | null;
  }

  /**
   * This method sets the necessary attributes and classes to the target element for the popover functionality.
   *
   * @returns void
   */
  private _setAttribute(): void {
    const targetElement = this._getFirstClickableElement();

    if (!targetElement || (targetElement && targetElement instanceof HTMLElement === false)) {
      return;
    }

    targetElement.setAttribute('popovertarget', `rng-popover-${this.rngPopoverTarget()}`);
    targetElement.classList.add('rng-popover__trigger');
    targetElement.style.setProperty('anchor-name', `--rng-${this.rngPopoverTarget()}`);
  }
}
